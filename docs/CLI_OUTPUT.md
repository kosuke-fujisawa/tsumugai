...
tsumugai compile scenario.md --target web --output bundle.json --no-assets
                                               # 実行前検査のアセットチェックを省略
```
...
## compile：出力（`--target web`）

`tsumugai compile <file> --target web --output <path>` は、実行前に check と同じ検査（routes 相当の全分岐探索を含む。[API.md 6.5章](API.md)）を行い、診断（error / warning とも）があれば check と同じ人間向け形式で stdout に表示します。error がなければ StoryBundle JSON を `--output` に書き出し、成功メッセージを表示します。

```text
StoryBundle を書き出しました: bundle.json (2 scenes, 3 assets)
```

- error（`circular-route` 含む）があれば StoryBundle を書き出さず、終了コード **1**
- warning のみなら warning を表示したうえで書き出し、終了コード **0**
- `--format` オプションはない（機械向け出力は `--output` に書き出す StoryBundle JSON そのもの）
- StoryBundle JSON の構造は [API.md 6.5章](API.md)、`schemaVersion` の扱いは [VERSIONING.md](VERSIONING.md) が正

---

## rule_id 一覧（check / trace / routes / fmt / compile）

SPEC.md 6章のルール表が正です。CLI はこれに加えて次を使います。

- `io-error`（error）: 記法ではなく環境の問題（ファイルが存在しない・読めない・ディレクトリを指定した 等）
- `circular-route`（error）: routes が検出した循環経路（SPEC 5.2）
- `unreachable-ending` / `unreachable-scene` / `route-without-ending` / `route-max-depth-exceeded` / `route-limit-exceeded`（いずれも warning）: routes が動的な探索でのみ検出できる到達可能性の問題（SPEC 5.2）
- routes 系の rule_id は、実行前検証に routes 相当の探索を含む `compile` からも報告される（[API.md 6.5章](API.md)）
...
```bash
tsumugai compile scenario.md --target renpy  # Ren'Py 変換（#79、未実装）
```

- `--target` は現在 `web` のみ対応。`renpy` を追加する場合も、実装済みの `--target web` と同じく実行前に check と同じ検査を行い、error があれば同じ形式で報告する方針（SPEC 6.1）
