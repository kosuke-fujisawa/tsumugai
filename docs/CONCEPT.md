...
tsumugai は npm ライブラリとして配布せず、CLI サブプロセス + JSON ファイルでホスト側と疎結合します（連携方法の詳細は [ARCHITECTURE.md](ARCHITECTURE.md) 8章）。
...
外部フロントエンド（arikoi の Svelte 製 player 等）とは、CLI サブプロセス + JSON ファイルで疎結合します。
