...
impl StoryEngine {
    ...
    pub async fn from_markdown(content: &str) -> Result<Self, StoryEngineError> {
        ...
        impl crate::application::use_cases::ScenarioRepositoryTrait for ScenarioRepositoryAdapter {
            async fn load_scenario(&self, id: &crate::domain::value_objects::ScenarioId) -> Result<crate::domain::entities::Scenario, crate::application::use_cases::RepositoryError> {
                use crate::domain::repositories::ScenarioRepository;
                self.inner.load_scenario(id)
                    .await
                    .map_err(|e| crate::application::use_cases::RepositoryError::ScenarioNotFound { 
                        id: id.as_str().to_string(),
                        source: Box::new(e)
                    })
            }
            
            async fn save_scenario(&self, scenario: &crate::domain::entities::Scenario) -> Result<(), crate::application::use_cases::RepositoryError> {
                use crate::domain::repositories::ScenarioRepository;
                self.inner.save_scenario(scenario)
                    .await
                    .map_err(|_| crate::application::use_cases::RepositoryError::AccessDenied { 
                        message: "Save not supported".to_string() 
                    })
            }
            
            async fn list_scenarios(&self) -> Result<Vec<crate::domain::value_objects::ScenarioId>, crate::application::use_cases::RepositoryError> {
                use crate::domain::repositories::ScenarioRepository;
                self.inner.list_scenarios()
                    .await
                    .map_err(|_| crate::application::use_cases::RepositoryError::AccessDenied { 
        ...
        Ok(Self::with_new_engine(execution, playback_use_case, new_engine))
    }

    /// Execute the next step in the story
    pub async fn step(&mut self) -> Result<StepResult, StoryEngineError> {
        let result = self.playback_use_case
    ...
    pub async fn save(&self) -> Result<(), StoryEngineError> {
        ...
        Err(StoryEngineError::configuration("Save functionality requires proper use case setup"))
    }
    ...
    pub async fn load(&mut self) -> Result<bool, StoryEngineError> {
        ...
        Err(StoryEngineError::configuration("Load functionality requires proper use case setup"))
    }
