...
mod domain_layer_tests {
    ...
    fn test_domain_error_types() {
        ...
        let result: Result<(), StoryExecutionError> = Err(StoryExecutionError::InvalidCommand {
...
mod application_layer_tests {
    ...
    fn test_use_case_error_propagation() {
        ...
        let use_case =
            ScenarioPlaybackUseCase::new(Box::new(failing_repository), Box::new(execution_service));
...
impl StoryExecutionService {
    ...
    fn execute_next_command(
        &self,
        _commands: &[Command],
        _pc: usize,
    ) -> Result<(), StoryExecutionError> {
        unimplemented!()
    }
}
...
impl ScenarioPlaybackUseCase {
    fn new(_repo: Box<dyn std::any::Any>, _service: Box<dyn std::any::Any>) -> Self {
        Self
    }
