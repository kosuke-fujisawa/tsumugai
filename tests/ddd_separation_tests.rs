...
mod ddd_tests {
    ...
    fn test_domain_pure_construction() {
        ...
        assert!(true, "Domain construction should be pure");
    }
    ...
    fn test_domain_error_quality() {
        ...
        match execution_result {
            Err(domain_error) => {
                ...
                assert!(error_string.contains("undefined"), "Error should mention undefined label");
    ...
    fn test_execution_service_branching() {
        ...
        let result1 = service.execute_next_command(&mut execution).expect("Should execute");
        match result1 {
        ...
        let result2 = service.execute_next_command(&mut execution).expect("Should execute");
        match result2 {
        ...
        let selected_label = service.select_branch_choice(&mut execution, 0).expect("Should select choice");
        assert_eq!(selected_label.as_str(), "left");

        // Execute next command (should be at left label)
        let result3 = service.execute_next_command(&mut execution).expect("Should execute");
        match result3 {
        ...
        let result4 = service.execute_next_command(&mut execution).expect("Should execute");
        match result4 {
    ...
    fn test_variable_operations_pure() {
        ...
        // Set variable
        let _result1 = service.execute_next_command(&mut execution).expect("Should set variable");
        
        // Modify variable  
        let _result2 = service.execute_next_command(&mut execution).expect("Should modify variable");
        
        // Check variable value
        let score = execution.state().get_variable(&VariableName::new("score".to_string()));
        assert_eq!(score, Some(&StoryValue::Integer(15)));

        // Execute conditional jump (should jump because 15 > 12)
        let result3 = service.execute_next_command(&mut execution).expect("Should execute conditional");
        match result3 {
    ...
    fn test_snapshot_pure() {
        ...
        let _result2 = service.execute_next_command(&mut execution).expect("Should execute say");
        
        // Verify we're at position 2
        assert_eq!(execution.state().program_counter(), 2);

        // Restore from snapshot
        service.restore_save_point(&mut execution, snapshot).expect("Should restore");
        
        // Should be back at position 1
        assert_eq!(execution.state().program_counter(), 1);
        
        // Variable should still be set
        let progress = execution.state().get_variable(&VariableName::new("progress".to_string()));
